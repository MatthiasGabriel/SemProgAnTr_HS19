...
\item binary clause
\end{itemize}
The empty clause contains no literal and in literature is denoted with different symbols. In this publication an uppercase lambda $\Lambda$  as suggested by Gomes et al \cite{Gomes2008SatisfiabilityS} will be used.
...
Even though the worst case still requires exponential time, normally these algorithms provide massive computational benefits. In this publication the recursive version of the algorithm will be used, because its much easier to understand some of the key ideas. Note that the algorithm also exists in iterative versions, which are able to reduce the memory usage \cite{Gomes2008SatisfiabilityS}.
