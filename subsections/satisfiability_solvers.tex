...
\begin{algorithm}[caption={UnitPropagate}, label={algUnitProgagate}]
 ...
 begin
   while $\Lambda \notin F$ and $\exists xClause \in F\mid IsUnitClause(xClause)$
	$\exists xLiteral \in xClause$
	while $\exists yClause \in F \mid xLiteral \in yClause$
		$F \gets F \setminus \{yClause\}$     
	while $\exists zClause \in F \mid \lnot xLiteral \in zClause$
