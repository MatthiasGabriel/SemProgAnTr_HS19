...
\centering
\caption{The flow chart of the unit propagate algorithm as desribed in algorithm \ref{algUnitProgagate}. Annotated with the actual values that are assigned for the input $F=(A \lor B ) \land (B \lor \lnot C) \land (C)$ and $\rho = \{\}$. The number on the left side is representing the current iteration.}
