...
\begin{tikzpicture}[
  ...
  text height=.8em,
  scale = 1.5,
  level 1/.style={sibling distance=10em, level distance = 25mm},
  level 2/.style={sibling distance=45em, level distance = 35mm},
...
\node[punkt] [rectangle split, rectangle split, rectangle split parts=3, text ragged] {
   	 \textbf{State A}
                  \nodepart{second}
            $\sigma = \{a\gets \alpha_a , b \gets \alpha_b\}$, $\pi = true$
                  \nodepart{third}
	Line 2: x = 1
}
    child {
	 ...
	 child {
		...
		Line 7: if($b<0$)
		 ...
		 child {
		        ...
		        edge from parent
		            node[kant, below, pos=.6] {$\alpha_b<0$}}
 		child {
		        node[punkt] [rectangle split, rectangle split, rectangle split parts=3, text ragged] {
			\textbf{State F}
		                  \nodepart{second}
		            $\sigma = \{a\gets \alpha_a +1, b \gets \alpha_b,x\gets 1\}$, $\pi =  \alpha_a \geq 0 \land \alpha_b > 0 \land \highlight{\alpha_b \geq 0}$
				\nodepart{third}
			Line 13: $assert(a!=0)$
		        }
			  child {
			        node[punkt] [rectangle split, rectangle split, rectangle split parts=1, text ragged] {
				\textbf{$\alpha_a+1 = 0 \land \alpha_a \geq 0 \land \alpha_b \leq 0 \land \alpha_b < 0 \land \alpha_b \geq 0 \Leftrightarrow false$}
			        }
		        edge from parent
		            node[kant, below, pos=.6] {}}
		        edge from parent
		            node[kant, below, pos=.6] {$\alpha_b \geq 0$}}
	        edge from parent
	            node[kant, below, pos=.6] {}}
        edge from parent
            node[kant, below, pos=.6] {$\alpha_b > 0$}}	
 ...
 child {
	...
	\textbf{State J}
                  \nodepart{second}
            $\sigma = \{a\gets \alpha_a , b \gets \alpha_b,x\gets 1\}$, $\pi = \alpha_a \geq 0 \land \alpha_b \leq 0 \land \highlight{\alpha_b \geq 0}$
		\nodepart{third}
	Line 13: $assert(a!=0)$
        }
        child {
	        node[punkt] [rectangle split, rectangle split, rectangle split parts=1, text ragged] {
		\textbf{$\alpha_a  = 0 \land \alpha_a \geq 0 \land \alpha_b \leq 0 \land \alpha_b \geq 0 \Leftrightarrow true$}
	        }
        edge from parent
            node[kant, below, pos=.6] {}}
        edge from parent
            node[kant, below, pos=.6] {$\alpha_b \geq 0$}}
        edge from parent
            node[kant, below, pos=.6] {$\alpha_b \leq 0$}}	
        edge from parent
            node[kant, below, pos=.6] {$\alpha_a \geq 0$}}
child {
	        node[punkt] [rectangle split, rectangle split, rectangle split parts=3, text ragged] {
		\textbf{State K}
	                  \nodepart{second}
	 ...
	 child {
        ...
        edge from parent
            node[kant, below, pos=.6] {}}
        edge from parent
            node[kant, below, pos=.6] {$\alpha_a < 0$}}	
        edge from parent
            node[kant, below, pos=.6] {}
    } ;
...
\todo{Add more specific description of the problems}

In the following chapter \ref{section:concolic_testing} these restrictions are discused and partially removed\todo{better word}.
...
\begin{tikzpicture}[
  ...
  level 3/.style={sibling distance=41em, level distance=25mm},
  level 4/.style={sibling distance=20em, level distance=25mm},
  level 5/.style={sibling distance=20em, level distance=35mm},
  level 6/.style={sibling distance=20em, level distance=25mm},
    edge from parent/.style={very thick,draw=blue!40!black!60,
        shorten >=5pt, shorten <=5pt},
    edge from parent path={(\tikzparentnode.south) -- (\tikzchildnode.north)},
    kant/.style={text width=2cm, text centered, sloped},
...
\node[punkt] [rectangle split, rectangle split, rectangle split parts=3, text ragged] {
    ...
    child {
		...
		\textbf{State B}
	                  \nodepart{second}
            	$\sigma = \{a\gets \alpha_a , b \gets \alpha_b, \highlight{q \gets 0}\}$, $\pi = true$
			\nodepart{third}
		Line 3: $while(a>=b)$
	        }
        child {
	...
	\textbf{State D}
                  \nodepart{second}
            $\sigma = \{a\gets \alpha_a , b \gets \alpha_b,q\gets 0\}$, $\pi = \highlight{\alpha_a \geq \alpha_b} $
		\nodepart{third}
	Line 4: $a = a -b$
        }
        child {
	...
	\textbf{State E}
                  \nodepart{second}
            $\sigma = \{\highlight{a\gets \alpha_a - \alpha_b}, b \gets \alpha_b,q\gets 0\}$, $\pi =  \alpha_a \geq \alpha_b$
		\nodepart{third}
	Line 5: $q = q + 1$
        }
	 child {
		...
		\textbf{State F}
	                  \nodepart{second}
	            $\sigma = \{a\gets \alpha_a - \alpha_b, b \gets \alpha_b,\highlight{q\gets 1}\}$, $\pi =  \alpha_a \geq \alpha_b$
			\nodepart{third}
		Line 3: $while(a>=b)$
	        }
		 child {
			...
			\textbf{State H}
	                     \nodepart{second}
		          $\sigma = \{a\gets \alpha_a-\alpha_b , b \gets \alpha_b,q\gets 0\}$, $\pi = \alpha_a \geq \alpha_b \land \highlight{\alpha_a-\alpha_b \geq \alpha_b} $
			\nodepart{third}
			Line 4: $a = a -b$
		        }
		        edge from parent
		            node[kant, below, pos=.6] {$\alpha_a-\alpha_b \geq \alpha_b$}}
 		child {
	        		node[punkt] [rectangle split, rectangle split, rectangle split parts=3, text ragged] {
			\textbf{State G}
	                  	\nodepart{second}
	            		$\sigma = \{a\gets \alpha_a , b \gets \alpha_b,q\gets 1\}$, $\pi =  \alpha_a \geq \alpha_b \land \highlight{\alpha_a-\alpha_b < \alpha_b}$
			\nodepart{third}
				Line 7: $return \text{ } q$
	       		 }
			  child {
        ...
        edge from parent
            node[kant, below, pos=.6] {}}		
        edge from parent
            node[kant, below, pos=.6] {$\alpha_a \geq \alpha_b$}}
child {
	        node[punkt] [rectangle split, rectangle split, rectangle split parts=3, text ragged] {
		\textbf{State C}
	                  \nodepart{second}
	            $\sigma = \{a\gets \alpha_a , b \gets \alpha_b,q\gets 0\}$, $\pi = \highlight{\alpha_a<\alpha_b}$
			\nodepart{third}
		Line 7: $return \text{ } q$
	        }
	 child {
        ...
        edge from parent
            node[kant, below, pos=.6] {}}
        edge from parent
            node[kant, below, pos=.6] {$\alpha_a < \alpha_b$}}	
        edge from parent
            node[kant, below, pos=.6] {}
    } ;
...
\end{tikzpicture}
\end{adjustbox}
\caption{Example of a partial symbolic execution tree based on the function displayed in codesnippet \ref{codeSnippet:symbolicExecutionLoop}.}
